...
TT.preloader.initialize = function () {
    ...
    spritesImage.src = hostPrefix + "/css/images/sprites.png" + versionSuffix;
    frontImage.src = hostPrefix + "/css/images/front-cover.jpg" + versionSuffix;
    backImage.src = hostPrefix + "/css/images/back-cover.jpg" + versionSuffix;
    rightImage.src = hostPrefix + "/css/images/right-page.jpg" + versionSuffix;
    leftImage.src = hostPrefix + "/css/images/left-page.jpg" + versionSuffix;
    repeatImage.src = hostPrefix + "/css/images/repeat-x.png" + versionSuffix;
    paperImage.src = hostPrefix + "/css/images/right-page-paper.jpg" + versionSuffix;
    leftFlippedImage.src = hostPrefix + "/css/images/left-page-flipped.jpg" + versionSuffix;
    backImageFlipped.src = hostPrefix + "/css/images/back-cover-flipped.jpg" + versionSuffix;
};
...
TT.storage.getArticlesFromServer = function () {
    ...
    $.ajax({
        url : hostPrefix + "/" + SERVER_VARIABLES.LANG + "/all",
        ...
        success : function (data) {
            ...
            $(data).each(function () {
                ...
                $(this).find("section").each(function (i) {
                    ...
                    if (TT.storage.supportsLocalStorage()) {
                        TT.storage.data.articles[hostPrefix + "/" + articleId + "/" + (i + 1)] = $("<div>").append($(this).clone()).remove().html();
                    }
...
TT.storage.setBookmark = function (articleId, pageNumber) {
    if (TT.storage.supportsLocalStorage() && articleId != TT.history.THEEND) {
        ...
        TT.storage.data.progress[hostPrefix + "/" + articleId + "/" + pageNumber] = true;
        TT.storage.save();
        TT.chapternav.updateReadMarkers();
        TT.tableofthings.updateReadMarkers();
    }
...
TT.storage.hasArticleBeenRead = function (articleId) {
    return TT.storage.data.progress[hostPrefix + "/" + articleId + "/1"] == true;
};
TT.pageflip = {};
...
TT.illustrations.ASSETS_URL = hostPrefix+"/media/illustrations/";
...
TT.navigation.goToHome = function (fromHistoryChange) {
    ...
    if (!TT.navigation.isHomePage()) {
        ...
        if (!fromHistoryChange) {
            TT.history.pushState(hostPrefix + "/" + TT.locale.getLocaleCodeFromURL() + "/home");
        }
...
TT.navigation.goToCredits = function (fromHistoryChange) {
    ...
    if (!TT.navigation.isCreditsPage() || (TT.navigation.isCreditsPage() && TT.navigation.isBookOpen())) {
        ...
        if (!fromHistoryChange) {
            TT.history.pushState(hostPrefix + "/" + TT.locale.getLocaleCodeFromURL() + "/credits");
        }
        TT.pageflip.turnToPage(currentPage, currentPage, 1, TT.pageflip.HARD_FLIP);
    } else {
...
TT.navigation.goToPage = function (articleId, pageNumber, fromHistoryChange) {
    ...
    if ((!isSamePageOverall && !isSamePageInBook) || (TT.navigation.isHomePage() || TT.navigation.isCreditsPage())) {
        ...
        if (currentGlobalPageNumber != null && targetGlobalPageNumber != null) {
            ...
            if (!fromHistoryChange) {
                TT.history.pushState(hostPrefix + "/" + TT.locale.getLocaleCodeFromURL() + "/" + articleId + "/" + pageNumber);
            }
...
TT.sharing.BASE_URL = l.protocol + "//" + l.hostname + hostPrefix;
...
TT.flipintro.render = function () {
    if (!TT.flipintro.canvas || !TT.flipintro.canvas[0]) {
        return;
    }
